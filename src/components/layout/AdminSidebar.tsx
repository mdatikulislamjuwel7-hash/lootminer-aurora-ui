...
import {
  LayoutDashboard, Users, Layers, ListChecks, Flame, Wallet,
  Ticket, FileText, Award, Settings, ScrollText,
} from "lucide-react";
...
export function AdminSidebar({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <aside className={`hidden md:flex sticky top-4 self-start h-[calc(100vh-2rem)] shrink-0 transition-[width] duration-300 ${collapsed ? "w-[72px]" : "w-[232px]"}`}>
      <div className="flex w-full flex-col rounded-3xl glass shadow-card p-3 overflow-hidden">
        {!collapsed && (
          <div className="px-2 pb-2 pt-1 text-[10px] uppercase tracking-widest text-muted-foreground">Admin</div>
        )}
